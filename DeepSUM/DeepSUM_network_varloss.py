##DJB June 2021
##Variation of DeepSUM network code
##This includes the variation loss function
##variation loss is also added to the tensorboard
## to use this function, change this line in DeepSUM_train.py: from DeepSUM_network_varloss import SR_network
##and update self.varlossfactor
...
class SR_network(object):
    def __init__(self,config):
        ...
        self.var=0
        self.varlossfactor = 3 ##this factor defines how much of a factor variation is
    ...
    def loss(self):
        ...
        with tf.name_scope('loss') as scope:
            ...
            hh = [-1,0,1]##to make this look at 5 x 5 pixel area rather than 3 x 3, change this to [-2,-1,0,1,2], for hh and kk
            kk = [-1,0,1]
            ...
            for h in hh:
                for k in kk:
                    label_moved = tf.manip.roll(labels, [h, k], [1,2])
                    label_moved = label_moved[:,self.border:(size_image-(2*self.border)),self.border:(size_image-(2*self.border))]
                    labels_moved.append(label_moved)
                    
                    pred_moved = tf.manip.roll(predictions, [h, k], [1,2])
                    pred_moved = pred_moved[:,self.border:(size_image-(2*self.border)),self.border:(size_image-(2*self.border))]
                    preds_moved.append(pred_moved)
                    
            stacked_labels = tf.stack(labels_moved, axis=3, name='stacked_labels')
            stacked_preds = tf.stack(preds_moved, axis=3, name='stacked_preds')
            
            meanlables, varlabels = tf.nn.moments(stacked_labels, 3)
            meanpreds, varpreds = tf.nn.moments(stacked_preds, 3)
            varlabels = tf.squeeze(varlabels,[3])
            varpreds = tf.squeeze(varpreds,[3])
            
            cropped_mask_y=self.mask_y[:,self.border:(size_image-(2*self.border)),self.border:(size_image-(2*self.border))]
            
            corrected_var=(self.varlossfactor/tf.reduce_sum(cropped_mask_y,axis=[2,3,4]))*tf.reduce_sum(tf.square(varlabels-varpreds),axis=[0,1,2])
            ...
            for i in range((2*self.border)+1):
                for j in range((2*self.border)+1):
                    ...
                    corrected_mse=(1.0/tf.reduce_sum(cropped_mask_y,axis=[2,3,4]))*tf.reduce_sum(tf.abs(cropped_labels_masked-corrected_cropped_predictions),axis=[1,2]) + (self.varlossfactor/tf.reduce_sum(cropped_mask_y,axis=[2,3,4]))*tf.reduce_sum(tf.square(varlabels-varpreds),axis=[0,1,2])
                    self.var = (self.varlossfactor/tf.reduce_sum(cropped_mask_y,axis=[2,3,4]))*tf.reduce_sum(tf.square(varlabels-varpreds),axis=[0,1,2])
    ...
    def summary(self):
        ...
        with tf.name_scope('performance') as scope:
            ...
            variance=tf.summary.scalar('variance',self.var, collections=['metric'])
        ...
        self.summary_var_metric=tf.summary.merge([variance])
        ...
        while True:
            ...
            for i in range(1,int(self.batch_training_norm.shape[0]/self.batch_size)+1):
                ...
                if (step+1)%200 == 0:
                    ...
                    _,l,summaries,summary_psnr_metric, summary_variance_metric=self.sess.run([self.opt,self.loss,self.summary_loss,self.summary_psnr_metric, self.var],feed_dict={
    ...
    def eval_once(self, writer, epoch, step):
        ...
        for i in range(1,int(self.batch_validation_norm.shape[0]/val_batch_size)+1):
            ...
            variance,score, psnr, loss, accuracy = self.sess.run([self.var,self.score, self.psnr, self.loss,self.accuracy],
                                           feed_dict={
        ...
        summary_average_variance = tf.Summary()
        summary_average_variance.value.add(tag="performance/diffinvar", simple_value=average_variance)
        ...
        writer.add_summary(summary_average_variance, global_step=step)
        ...
        print('difference in variance on test batch at epoch {0}: {1} '.format(epoch, average_variance))
        
        print('Took: {0} seconds'.format(time.time() - start_time))

        
    def predict_test(self,test_dir,n_slide, setsize, mu, sigma, band):
        '''
        ...
        input_images_LR_test=np.load(os.path.join(test_dir,'dataset_{0}_LR_test.npy'.format(band.upper())),allow_pickle=True)
        print ("got images 1")
        mask_LR_test=np.load(os.path.join(test_dir,'dataset_{0}_mask_LR_test.npy'.format(band.upper())),allow_pickle=True)
        print ("got images 2")
        shift_LR_test=np.load(os.path.join(test_dir,'shifts_test_{0}.npy'.format(band.upper())),allow_pickle=True)
        print ("got images 3")
        LRmap = np.load(os.path.join(test_dir,'dataset_{0}_LRmap.npy'.format(band.upper())),allow_pickle=True)
        print ("got images")
        ...
        ckpt = tf.train.get_checkpoint_state(os.path.dirname('checkpoints/'+band+'/checkpoint'))
        if ckpt and ckpt.model_checkpoint_path:
            saver.restore(self.sess, ckpt.model_checkpoint_path)

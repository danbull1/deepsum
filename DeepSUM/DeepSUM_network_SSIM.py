...
##This includes the SSIM and MS-SSIM loss function
##SSIM is also added to the tensorboard
## to use this function, change this line in DeepSUM_train.py: from DeepSUM_network_SSIM import SR_network
##note requires tf 1.15 to run due to requirements of SSIM
...
class SR_network(object):
        ...
        with tf.name_scope('loss') as scope:
            ...
            for i in range((2*self.border)+1):
                for j in range((2*self.border)+1):
                    cropped_labels=labels[:,i:i+(size_image-(2*self.border)),j:j+(size_image-(2*self.border))] ##(8, 96, 96, 1)
                    ...
                    corrected_memse=(1.0/tf.reduce_sum(cropped_mask_y,axis=[2,3,4]))*tf.reduce_sum(tf.abs(cropped_labels_masked-corrected_cropped_predictions),axis=[1,2])
                    
                    cropped_mask_y = tf.squeeze(cropped_mask_y,axis=1)
                    
                    stack_tn = tf.stack([cropped_labels_masked, corrected_cropped_predictions, cropped_mask_y],1)
                    ssims = []
                    
                    ##DJB function to iterate through batch
                    def fn(stack_tn):
                      cropped_labels_masked_single = stack_tn[0]
                      corrected_cropped_predictions_single = stack_tn[1]
                      cropped_mask_y_single = stack_tn[2]
                      #ssim = 1 - tf.image.ssim(cropped_labels_masked_single, corrected_cropped_predictions_single, max_val=16384)
                      #https://stackoverflow.com/questions/57127626/error-in-calculation-of-inbuilt-ms-ssim-function-in-tensorflow
                      ssim = 1 - tf.image.ssim(cropped_labels_masked_single, corrected_cropped_predictions_single, max_val=255,filter_size=11, filter_sigma=3, k1=0.01, k2=0.03)
                      #ssim = 1- tf.image.ssim_multiscale(cropped_labels_masked_single, corrected_cropped_predictions_single, max_val=255, power_factors=(0.0448, 0.2856, 0.3001, 0.2363, 0.1333), filter_size=6,filter_sigma=2, k1=0.01, k2=0.03)
                      ssim = tf.dtypes.cast(ssim, tf.float32)
                      
                      weighted_ssim = tf.reduce_sum(cropped_mask_y_single,axis=[0,1,2])/9934*ssim
                     
                      #print_op = tf.print(ssim,output_stream=sys.stdout)
                      #with tf.control_dependencies([print_op]):
                        #self.out = tf.add(ssim, ssim)

                      
                      return weighted_ssim
                      
                    map_result = tf.map_fn(fn, stack_tn, dtype=tf.float32)
                    ssims_mean=tf.reduce_mean(map_result,axis=0)
                    
                    
                    #ave_loss = (ssims_mean + corrected_memse)/2
                    X.append(corrected_memse)
    ...
    def PSNR(self,norm=True):
        ...
        for i in range((2*self.border)+1):
            for j in range((2*self.border)+1):
                ...
                def fn(stack_tn):
                  ...
                  corrected_cropped_predictions_single = stack_tn[1]
                  cropped_mask_y_single = stack_tn[2]
                  ssim = tf.image.ssim(cropped_labels_masked_single, corrected_cropped_predictions_single, max_val=256)
                  ssim = tf.dtypes.cast(ssim, tf.float32)
                  
                  ##weighted_ssim = tf.reduce_sum(cropped_mask_y_single,axis=[0,1,2])/9934*ssim*1000000
                  
                  return ssim
                  
                map_result = tf.map_fn(fn, stack_tn, dtype=tf.float32)
                ssims_mean=tf.reduce_mean(map_result,axis=0)
                Xssim.append(ssims_mean)
                

        X=tf.stack(X)
        Xssim = tf.stack(Xssim)
        ...
        if norm:
            ...
            max_cPSNR=tf.reduce_max(X,axis=0)
            max_cSSIM=tf.reduce_max(Xssim,axis=0)
            psnr=tf.reduce_mean(max_cPSNR)
            ssim=tf.reduce_mean(max_cSSIM)
            return psnr,ssim
    ...
    def summary(self):
        ...
        with tf.name_scope('performance') as scope:
            ...
            ssim=tf.summary.scalar('ssim',self.ssim, collections=['metric'])
        ...
        self.summary_ssim_metric=tf.summary.merge([ssim])
        ...
        while True:
            ...
            for i in range(1,int(self.batch_training_norm.shape[0]/self.batch_size)+1):
                ...
                if (step+1)%200 == 0:
                    ...
                    _,l,summaries,summary_psnr_metric,summary_ssim_metric=self.sess.run([self.opt,self.loss,self.summary_loss,self.summary_psnr_metric, self.summary_ssim_metric],feed_dict={
    ...
    def eval(self):
        ...
        with tf.name_scope('psnr'):
            ...
            self.psnr, self.ssim=self.PSNR(norm=False)
    ...
    def eval_once(self, writer, epoch, step):
        ...
        for i in range(1,int(self.batch_validation_norm.shape[0]/val_batch_size)+1):
            ...
            score, psnr, loss, accuracy, ssim = self.sess.run([self.score, self.psnr, self.loss,self.accuracy, self.ssim],
                                           feed_dict={
        ...
        summary_average_ssim = tf.Summary()

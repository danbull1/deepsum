##DJB June 2021
##perception loss function integrated with DeepSUM network code
##This uses the style.py class and autoencoder keras model (included) trained on aerial imagery
## to use this function, change this line in DeepSUM_train.py: from DeepSUM_network_perceptionloss import SR_network

##also need to add these config items. Weights are as used (tv weight not used)
...
from style import StyleContentModel
...
class SR_network(object):
    def __init__(self,config):
        ...
        #configmem.gpu_options.per_process_gpu_memory_fraction = 0.9

        configmem.gpu_options.allow_growth=True
        self.sess=tf.Session(config=configmem)
      
        self.styleweight = config['styleweight']
        self.contentweight = config['contentweight']
        self.tvweight = config['tvweight']
...
#loss
        self.vgg_path = r"C:\DeepLearning\COMPX594\DeepSUM\imagenet-vgg-verydeep-19.mat"
        self.s_loss = 0
        self.c_loss = 0
        self.t_loss = 0
        
        self.vgg_target = None
        #self.autoencodermodel = '/home/db129/SuperResolution/DeepSUM_PL/autoencode3'
        self.autoencodermodel = '/home/db129/SuperResolution/DeepSUM_PL/autoencode5'
    ...
    def get_data(self):
        with tf.name_scope('data'):
            ...
            ##DJB can't round data hereon on win
            ...
            for i in range(0,numimtokeep):#DJB only 3 images per set
                self.fill_coeff_train[:,:,i]=np.expand_dims(self.batch_training_mask[:,i],axis=1)

            for i in range(0,numimtokeep):
                for j in range(i+1,numimtokeep):
                    rows_indexes=[k for k in range(0,numimtokeep) if k!=(j)]
                    #print(rows_indexes)
            ...
            #[b,9,W,H,1]
            self.fill_coeff_train[:,range(numimtokeep),range(numimtokeep),:,:,:]=self.fill_coeff_train[:,range(numimtokeep),range(numimtokeep),:,:,:]+np.logical_not(f)[:,range(numimtokeep),:,:,:]
            ##line 187, in get_data
            ...
            #[b,9,W,H,1]
            self.fill_coeff_valid[:,range(numimtokeep),range(numimtokeep),:,:,:]=self.fill_coeff_valid[:,range(numimtokeep),range(numimtokeep),:,:,:]+np.logical_not(f)[:,range(numimtokeep),:,:,:]
   ...
   ##vgg loss function         
    def loss(self):
        ...
        with tf.name_scope('loss') as scope:
            content_layers = ['block1_conv1','block1_conv2', 'block4_conv1','block4_conv2']

            style_layers = ['block1_conv1','block2_conv1', 'block2_conv2','block1_conv2']
            ...
            extractor = StyleContentModel(style_layers, content_layers, self.autoencodermodel)
            
            self.y_hat=self.logits
            labels = (labels*self.sigma + self.mu)/255


            cropped_mask_y=self.mask_y
            predictions_masked=predictions*tf.squeeze(cropped_mask_y,axis=1)
            
            #bias brightness
            b=(1.0/tf.reduce_sum(cropped_mask_y,axis=[2,3,4]))*tf.reduce_sum(labels-predictions_masked,axis=[1,2])
            b=tf.reshape(b,[s1[0],1,1,1])
            corrected_predictions=predictions_masked+b
            corrected_predictions=corrected_predictions*tf.squeeze(cropped_mask_y,axis=1)
    
            predictions=(corrected_predictions*self.sigma + self.mu)/255
            #predictions_3d = tf.concat([predictions, predictions, predictions], 3)
            #ground_truth_3d = tf.concat([labels, labels, labels], 3)


            # Extract style target 
            style_target = extractor(labels)['style']#*255
            style_output = extractor(predictions)['style']
            ...
            content_target = extractor(labels)['content']
            content_output = extractor(predictions)['content']
            
            #self.vgg_target = tf.print(content_target,output_stream=sys.stdout)
            #self.vgg_target  = content_target['block4_conv2']
            #outputs = extractor(predictions_3d)
            #print(print_op)

            content_weight = self.contentweight
            style_weight = self.styleweight
            tv_weight = self.tvweight
            self.s_loss = style_loss(style_output, style_target) * style_weight

            self.c_loss = content_loss(content_output, content_target) * content_weight
 
            #self.t_loss = tv_weight * tf.reduce_sum(total_variation_loss(output_image))
            
            self.loss = self.c_loss# + self.s_loss
    ...
    def inference_FR(self):
        ...
        #################  FUSION NET    ##################################
         ##DJB chnaged 1,2,3 dim to 2,2,2 from 3,3,3
         #DJB updated to take 9 in image set (18/12/2020)
        x1 = Conv3D(tf.pad(self.x_to_fuse, sp, mode='REFLECT'), [2,3,3,F,F1], [1,1,1,1,1], 'VALID', scope_name='conv_fuse_0',trainable=self.TRAIN_FUSION_NET)
        x1=InstanceNorm(x1,axis=[2,3],name='inst_norm_fuse_0',trainable=self.TRAIN_FUSION_NET)
        x1 = tf.nn.leaky_relu(x1)
        
        x1 = Conv3D(tf.pad(x1, sp, mode='REFLECT'), [2,3,3,F1,F1], [1,1,1,1,1], 'VALID', scope_name='conv_fuse_1',trainable=self.TRAIN_FUSION_NET)
        x1=InstanceNorm(x1,axis=[2,3],name='inst_norm_fuse_1',trainable=self.TRAIN_FUSION_NET)
        x1 = tf.nn.leaky_relu(x1)
        
        ##DJB chnaged 1st dim to 1 from 3
        x1 = Conv3D(tf.pad(x1, sp, mode='REFLECT'), [2,3,3,F1,F1], [1,1,1,1,1], 'VALID', scope_name='conv_fuse_2',trainable=self.TRAIN_FUSION_NET)
    ...
    def summary(self):
        ...
        with tf.name_scope('performance') as scope:
            ...
            contentloss=tf.summary.scalar('contentloss',self.c_loss, collections=['metric'])
            styleloss=tf.summary.scalar('styleloss',self.s_loss, collections=['metric'])
        ...
        self.summary_contentloss_metric=tf.summary.merge([contentloss])
        self.summary_styleloss_metric=tf.summary.merge([styleloss])
    ...
    def train_one_epoch(self,saver,train_writer,test_writer,epoch,step):
        ...
        if self.full:
            self.gen=load_dataset(self.dataset_path,self.n_chunks,self.spectral_band,num_images=8,how='best')
        else:
        ...
        while True:
            ...
            for i in range(1,int(self.batch_training_norm.shape[0]/self.batch_size)+1):
                ...
                if (step+1)%200 == 0:
                    ...
                    content_loss, style_loss,_,l,summaries,summary_psnr_metric=self.sess.run([self.summary_contentloss_metric, self.summary_styleloss_metric,self.opt,self.loss,self.summary_loss,self.summary_psnr_metric],feed_dict={
                    ...
                    train_writer.add_summary(content_loss, global_step=step)
                    train_writer.add_summary(style_loss, global_step=step)
                    train_writer.flush()
                ...
                if (step+1)%self.skip_step==0:
                    ...
                    #print('content loss is: ' + str(content_loss))
                    #print('style loss is: ' + str(style_loss))
    ...
    def train(self,n_epochs):
        ...
        saver = tf.train.Saver()##{##all layers}
    ...
    def eval_once(self, writer, epoch, step):
        ...
        for i in range(1,int(self.batch_validation_norm.shape[0]/val_batch_size)+1):
            ...
            contentloss, styleloss, score, psnr, loss, accuracy = self.sess.run([self.c_loss, self.s_loss, self.score, self.psnr, self.loss,self.accuracy],
                                           feed_dict={
            ...
            contentloss_list.append(contentloss)
            styleloss_list.append(styleloss)
        ...
        average_contentloss=np.mean(contentloss_list)
        average_styleloss=np.mean(styleloss_list)
        ...
        summary_average_contentloss = tf.Summary()
        summary_average_contentloss.value.add(tag="performance/contentloss", simple_value=average_contentloss)
        
        summary_average_styleloss = tf.Summary()
        summary_average_styleloss.value.add(tag="performance/styleloss", simple_value=average_styleloss)
        ...
        writer.add_summary(summary_average_contentloss, global_step=step)
        writer.add_summary(summary_average_styleloss, global_step=step)
        ...
        print('PSNR on test batch at epoch {0}: {1} '.format(epoch, average_psnr))
        print('content loss on test batch at epoch {0}: {1} '.format(epoch, average_contentloss))
        print('style loss on test batch at epoch {0}: {1} '.format(epoch, average_styleloss))
        
        print('Took: {0} seconds'.format(time.time() - start_time))

        
    def predict_test(self,test_dir,n_slide, setsize):
        '''
        ...
        print ("got images 3")
        LRmap = np.load(os.path.join(test_dir,'dataset_{0}_LRmap.npy'.format(self.spectral_band)),allow_pickle=True)
        print ("got images")
        ...
        #val_batch_size=1
        SR_images=np.zeros([len(input_images_LR_test),1,512,512,1])
        for m in range(0,len(input_images_LR_test)):
            ...
            percentage=0.9
            ##DJB keps reducing the amount fo cloud cover until there are three in set
            while True:
                indexes_0=np.argwhere((np.sum(imageset_mask[0:],axis=(1,2))/(512*512))>percentage).squeeze(axis=1)
                indexes_0=indexes_0 if indexes_0.ndim>0 else np.array([]) 
                
                
                indexes=np.array(list(indexes_0))
                if indexes.size>=setsize:
            ...
            len_imageset=np.shape(imageset)[0] ##sshould be 6
            
            temporal_dim=setsize
            upper_bound=n_slide
            ...
            SR_imageset=np.zeros([size,1,512,512,1]) ##DJB
            for n in range(0,size):
                ...
                for i in range(0,setsize):
                    ##x = np.expand_dims(imageset_9_mask[:,i],axis=1)
                    fill_coeff_test[:,:,i]=np.expand_dims(imageset_9_mask[:,i],axis=1)
        
                for i in range(0,setsize):
                    for j in range(i+1,setsize):
                        rows_indexes=[k for k in range(0,setsize) if k!=(j)]
                        #print(rows_indexes)
                ...
                #[b,9,W,H,1]
                fill_coeff_test[:,range(setsize),range(setsize),:,:,:]=fill_coeff_test[:,range(setsize),range(setsize),:,:,:]+np.logical_not(f)[:,range(setsize),:,:,:]
                    
                ####################
                
                ##DJB images normliased in preparation
            ...
            SR_imageset_registered=np.empty([0,1,512,512,1])
            for z in range(SR_imageset.shape[0]):
                ...
                corrected_image = corrected_image.reshape([1,1,512,512,1])
